...
console.log(KareninAlani(10)); // Örnek görev

function CemberinCevresi(yaricap) {
  return 2 * pi * yaricap;
}

console.log(CemberinCevresi(5)); // Görev 1

function CemberinAlani(yaricap, piSayisi) {
  return piSayisi * Math.pow(yaricap, 2);
}

console.log(CemberinAlani(15, pi)); // Görev 2
...
console.log("En küçük sayı:", enkucuk); // Görev 3a
console.log("En büyük sayı:", enbuyuk); // Görev 3a
...
console.log("3'e tam bölünenler:", ucetambolunenler); // Görev 3b

let ucebolunenlerintoplami = ucetambolunenler.reduce((toplam, sayi) => toplam + sayi, 0);

console.log("3'e tam bölünenlerin toplamı:", ucebolunenlerintoplami); // Görev 3c

let besyuzdenkucuksayilar = sayilar.filter((sayi) => sayi < 500);

console.log("500'den küçük sayılar:", besyuzdenkucuksayilar); // Görev 3d

let siralisayilar = [...besyuzdenkucuksayilar].sort((a, b) => a - b);

console.log("Sıralı sayılar:", siralisayilar); // Görev 3e

let sayiTekrar = {};
let tekraredensayilar = [];

// Sayıların tekrarını saymak
sayilar.forEach((sayi) => {
  sayiTekrar[sayi] = (sayiTekrar[sayi] || 0) + 1;
});

// Tekrar eden sayıları string formatında oluşturmak
for (let [sayi, tekrarSayisi] of Object.entries(sayiTekrar)) {
  if (tekrarSayisi > 1) {
    tekraredensayilar.push(`${sayi} sayısı ${tekrarSayisi} kere tekrar edilmiştir`);
  }
}

console.log("Tekrar eden sayılar:", tekraredensayilar); // Görev 3f

console.log(`Toplam sayı adedi: ${sayilar.length}`); // Ek bilgi: Dizide kaç sayı olduğunu yazdırma
